...
// Middleware to prevent body-parser from hanging if body is already parsed by Vercel serverless runtime
...
apiRouter.post('/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    if (loginType === UserRole.SUPER_ADMIN || loginType === 'SUPER_ADMIN' || loginType === 'super_admin') {
      const user = inMemoryStore.users.find(
        (u) =>
          u.role === UserRole.SUPER_ADMIN &&
          (u.email.toLowerCase() === (email || '').trim().toLowerCase() || u.email === 'matoa@gmail.com')
      );
    ...
    if (loginType === UserRole.AUTO_ECOLE_ADMIN || loginType === 'AUTO_ECOLE_ADMIN' || loginType === 'auto_ecole_admin') {
      const user = inMemoryStore.users.find(
    ...
    if (loginType === UserRole.ELEVE || loginType === 'ELEVE' || loginType === 'eleve') {
      if (!codeAutoEcole) {
...
apiRouter.delete('/auto-ecoles/:id', authMiddleware, requireRoles(UserRole.SUPER_ADMIN), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const ae = inMemoryStore.getAutoEcoleById(id);
    if (!ae) {
